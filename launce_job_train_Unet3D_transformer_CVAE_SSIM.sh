...
for alpha2 in "${alphas[@]}"; do
  for beta in "${betas[@]}"; do
      ...
      job_script="submit_alpha_${alpha}_alpha2_${alpha2}_beta_${beta}.sh"
...
python Training/train_Unet3D_transformer_CVAE_SSIM.py --alpha ${alpha} --alpha2 ${alpha2} --beta ${beta}
EOF
