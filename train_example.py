...
f = open(f"only_unet_batchsize_64_hidsize_256_lr_5e-4_wd_1e-6.txt", "w")
...
if __name__ == '__main__':
    ...
    # toy_df = toy_df.iloc[:len(toy_df) // 5]
    ...
    f.write("Initial train loss-%.7f\n" % (initial_train_loss))
    f.write("Initial val loss-%.7f\n" % (initial_val_loss))
    ...
    for epoch in range(epochs):
        ...
        f.write("Epoch-%d, train loss-%.7f, val loss-%.7f\n" % (epoch, epoch_train_loss, epoch_val_loss))
...
f.close()
import joblib
joblib.dump(model, 'only_unet_batchsize_64_hidsize_256_lr_5e-4_wd_1e-6.pkl')
