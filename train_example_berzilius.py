...
if __name__ == '__main__':
    ...
    # base_model_name = "only_final_unet_without_final_ReLU_L1loss_norm_minmax_maps_forward_model_bad_nconfs3_to_good_res2.0_batchsize_64_hidsize_256_levels_256_lr_5e-4_wd_1e-5"
    
    toy_df = pd.read_csv(os.path.join(toy_dir, "PDB_IDs_with_rdkit_length_less_than_24A.csv"))
    n_splits = 10
    ...
    base_model_name = f"k_{k}_only_final_unet_with_final_ReLU_L2loss_norm_minmax_maps_forward_model_bad_nconfs3_to_good_res2.0_batchsize_64_hidsize_256_levels_256_lr_5e-4_wd_1e-5"
    ...
    for epoch in range(epochs):
        ...
        epoch_val_loss = val(model, valid_loader, device, val_criterion)

        msg = "epoch-%d, train_loss-%.7f, val_loss-%.7f" \
        % (epoch, epoch_train_loss, epoch_val_loss)
        logger.info(msg)

        if (epoch >= 2) and (epoch_val_loss < best_val_loss):
    ...
    #         logger.info(msg)
            ...
            # #start validating
            # valid_rmse, valid_pr = val(model, valid_loader, device, val_criterion)
