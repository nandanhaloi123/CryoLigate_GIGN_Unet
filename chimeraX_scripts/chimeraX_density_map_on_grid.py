...
"""
This script is used to compute density map for the given molecule on the grid of the given (target) map 
inside ChimeraX. 
The script accepts the following input arguments:
-i: full path to the input molecule file
-t: full path to the target map which grid will be used to compute the molecule's map
-r: resolution for the molecule map generation
-o: full path to the molecule's output density file
-l: if provided, contains path to the folder where log file will be written
"""

opts, args = getopt.getopt(sys.argv[1:], "i:t:r:o:l:")

molecule_path_full = None  # full path to the input molecule file (including its name)
target_map_path_full = None # full path to the target map (including its name)
density_resolution = None  # density map resolution for the molecule (in Angrstrom)
output_density_path_full = None  # full path to the molecule's output density file
...
for opt, arg in opts:
    if opt == "-i":
        ...
        target_map_path_full = arg
    elif opt == "-r":
        density_resolution = arg
    elif opt == "-o":
        output_density_path_full = arg
    elif opt == "-l":
        is_log = True
        log_path = arg
...
), "Path to the target map file is None after argument's parsing."
...
run_com(
    session,
    "open " + target_map_path_full,
    is_log=is_log,
    log_path=log_path,
    log_filename=log_fname,
)  # open target density file

run_com(
    session,
    "molmap #1 {} onGrid #2".format(density_resolution),
    is_log=is_log,
    log_path=log_path,
    log_filename=log_fname,
)  # generate density map for the molecule on the grid of the target map

run_com(
    session,
    "save {} #3".format(output_density_path_full),
    is_log=is_log,
    log_path=log_path,
    log_filename=log_fname,
)  # save the map
