...
class CryoLigateVAE(nn.Module):
    def __init__(self, latent_dim=512):
        ...
        self.Encoder = UNet3DEncoder(in_channels=2)
        self.ReLU = nn.ReLU(inplace=True)  # Use inplace ReLU to save memory

        # Latent space transformation
        self.fc_mu = nn.Linear(512 * 6 * 6 * 6, latent_dim)
        self.fc_logvar = nn.Linear(512 * 6 * 6 * 6, latent_dim)
        self.fc_decode = nn.Linear(latent_dim, 512 * 6 * 6 * 6)
    ...
    @autocast()  # Use mixed precision to save memory
    def forward(self, data):
        ...
        x = torch.cat((x, low_res_dens), dim=1)
        ...
        x = self.fc_decode(z).view(-1, 512, 6, 6, 6)
...
class MLP1DTo3D(nn.Module):
    def __init__(self, input_size, output_shape):
        ...
        level_channels = [128, 128 * 8, 128 * 16]  # Reduced size to save memory
...
class Conv3DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, apply_transformer=True, bottleneck=False):
        ...
        self.relu = nn.ReLU(inplace=True)  # Use inplace ReLU
...
class UNet3DEncoder(nn.Module):
    def __init__(self, in_channels=2, level_channels=[64, 128, 256], bottleneck_channel=512):
        super().__init__()
...
class UNet3DDecoder(nn.Module):
    def __init__(self, level_channels=[64, 128, 256], bottleneck_channel=512):
        super().__init__()
