...
class CryoLigateVAE(nn.Module):
    def __init__(self):
        ...
        self.UNet3DTransformerVAE = UNet3DTransformerVAE(in_channels=2, latent_dim=latent_dim)
        self.ReLU = nn.ReLU()
    ...
    def forward(self, data):
        ...
        x, mu, logvar = self.UNet3DTransformerVAE(x)
        x = self.ReLU(x)
        return x, mu, logvar
...
class UNet3DTransformerVAE(nn.Module):
    def __init__(self, in_channels=1, level_channels=[64, 128, 256], bottleneck_channel=512, latent_dim=256):
        super().__init__()
        ...
        self.fc_mu = nn.Linear(bottleneck_channel * 6 * 6 * 6, latent_dim)
        self.fc_logvar = nn.Linear(bottleneck_channel * 6 * 6 * 6, latent_dim)
        self.fc_decode = nn.Linear(latent_dim, bottleneck_channel * 6 * 6 * 6)
...
class Discriminator3D(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(128 * 6 * 6 * 6, 1),  # Adjust according to input size
            nn.Sigmoid()
        )
